...
const LargeFeatureDescript: FunctionComponent<LargeFeatureDescriptProps> = ({
  ...
  return (
    <article className={`flex ${rowOrder} justify-end gap-10 lg:gap-20 items-center hidden md:flex`}>
      <div className={`flex flex-col ${textAlign} ${textSpace} gap-5 max-w-md`}>
        <header>
          <h1 className="font-primary-font text-white text-3xl lg:text-5xl font-normal">{title}</h1>
        </header>
        <p className="max-w-4xl text-base lg:text-lg md:max-w-xl text-light-gray">{desc}</p>
      </div>
      {children}
    </article>
  );
